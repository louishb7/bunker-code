...
import { removePlannedClaim, removePlannedOpenQuestion, removePlannedPart, removePlannedPredicate, removePlannedRelation,
...
interface InspectorProps {
  model: PlannedSystemModel; selection: PlannedSubjectRef; editor: DesignerEditor;
...
export function DesignerInspector({ model, selection, editor, commit, select, closeEditor, connect }: InspectorProps) {
  ...
  return <aside className="designer-inspector" aria-label="Designer inspector" data-inspector-subject={selection.kind}>
    ...
    {part ? <>
      ...
      <section><div className="designer-section-heading"><h3>Relations</h3><button onClick={() => connect(part.id)}>Connect Part</button></div>
        {model.relations.filter((item) => item.sourcePartId === part.id || item.targetPartId === part.id).map((item) => <button className="designer-relation-link" key={item.id} onClick={() => select({ kind: 'relation', relationId: item.id })}>{relationLabel(model, item)}</button>)}
      </section>
    </> : relation ? <>
      ...
      <details className="designer-predicates"><summary>Predicates · {model.predicates.length}</summary><p className="designer-muted">Definitions belong to this system. Create one while connecting Parts.</p>
        {model.predicates.map((predicate) => <PredicateEditor key={predicate.id} predicate={predicate} model={model} commit={commit} />)}
      </details>
    </>}
    <Annotations key={JSON.stringify(selection)} model={model} subject={selection} commit={commit} />
    {part && <button className="designer-danger" onClick={() => {
      const relations = model.relations.filter((item) => item.sourcePartId === part.id || item.targetPartId === part.id);
      if (window.confirm(`Delete “${part.label}”? Its ${relations.length} connected Relations and all Claims and Open Questions attached to this Part or those Relations will also be deleted.`) && commit((current) => removePlannedPart(current, part.id))) select({ kind: 'model' });
    }}>Delete Part</button>}
    {relation && <button className="designer-danger" onClick={() => {
      if (window.confirm('Delete this Relation and its Claims and Open Questions?') && commit((current) => removePlannedRelation(current, relation.id))) select({ kind: 'model' });
...
function RelationForm({ model, relation, source, target, commit, saved }: { model: PlannedSystemModel; relation?: PlannedRelation; source?: string; target?: string; commit: CommitModel; saved(id: string): void }) {
  ...
  return <Form label={relation ? 'Save Relation' : 'Add Relation'} submit={(data) => {
    ...
    <label className="designer-field">Predicate<select name="predicate" value={predicateChoice} onChange={(event) => setPredicateChoice(event.target.value)}>
      ...
      <optgroup label="Starters — editable definitions">{starters.map((starter, index) => <option key={starter.label} value={`starter:${index}`}>{starter.label}</option>)}</optgroup>
    ...
    <label className="designer-field">Target Part<select name="target" defaultValue={relation?.targetPartId ?? target ?? model.parts.find((part) => part.id !== source)?.id} required>{model.parts.map((part) => <option key={part.id} value={part.id}>{part.label}</option>)}</select></label>
    <Field name="description" label="Description" value={relation?.description} multiline /><AuthorFields value={relation} />
  </Form>;
}
function PredicateEditor({ predicate, model, commit }: { predicate: PlannedPredicateDefinition; model: PlannedSystemModel; commit: CommitModel }) {
  const used = model.relations.filter((item) => item.predicateId === predicate.id).length;
  return <details><summary>{predicate.label} · {used} Relations</summary>
...
function Annotations({ model, subject, commit }: { model: PlannedSystemModel; subject: PlannedSubjectRef; commit: CommitModel }) {
  ...
  return <div className="designer-annotations">
    <section><div className="designer-section-heading"><h3>Claims <span>{claims.length}</span></h3><button onClick={() => setEditing({ kind: 'claim' })}>Add Claim</button></div>
      ...
      {claims.map((claim) => <article className="designer-note" key={claim.id}><span className={`designer-modality ${claim.modality}`}>{claim.modality}</span><p>{claim.statement}</p>
        ...
        <button aria-label={`Edit Claim: ${claim.statement}`} onClick={() => setEditing({ kind: 'claim', id: claim.id })}>Edit</button>{' '}
        <button aria-label={`Delete Claim: ${claim.statement}`} onClick={() => { if (window.confirm('Delete this Claim?') && commit((current) => removePlannedClaim(current, claim.id)) && editing?.kind === 'claim' && editing.id === claim.id) setEditing(null); }}>Delete</button>
      </article>)}
    </section>
    <section><div className="designer-section-heading"><h3>Open Questions <span>{questions.length}</span></h3><button onClick={() => setEditing({ kind: 'question' })}>Add Question</button></div>
      ...
      {questions.map((question) => <article className="designer-note" key={question.id}><p>{question.question}</p>
        ...
        <button aria-label={`Edit Question: ${question.question}`} onClick={() => setEditing({ kind: 'question', id: question.id })}>Edit</button>{' '}
        <button aria-label={`Delete Question: ${question.question}`} onClick={() => { if (window.confirm('Delete this Open Question?') && commit((current) => removePlannedOpenQuestion(current, question.id)) && editing?.kind === 'question' && editing.id === question.id) setEditing(null); }}>Delete</button>
      </article>)}
    </section>
    {editing && <section className="designer-note-editor" key={`${editing.kind}:${editing.id ?? 'new'}`}>
      <div className="designer-section-heading"><h3>{editing.id ? 'Edit' : 'New'} {editing.kind === 'claim' ? 'Claim' : 'Open Question'}</h3><button onClick={() => setEditing(null)}>Cancel</button></div>
