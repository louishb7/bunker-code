...
async function click(page: Page, label: string, scope = '') {
  ...
  const handle = await page.evaluateHandle((label, scope) => [...document.querySelectorAll(`${scope} button`)].find((button) => button.textContent?.trim() === label || button.firstChild?.textContent?.trim() === label), label, scope);
...
test('DESIGN authors a system without a snapshot, persists layout and intent, and validates import/export in a browser', { timeout: 180000 }, async (t) => {
  ...
  await page.type('[aria-label="Find Part"]', 'Users'); await click(page, 'Users', '.designer-search-results');
  await click(page, 'Implementation', '.designer-inspector');
  await fill(page, '[aria-label="Find technology"]', 'TypeScript'); await page.focus('[aria-label="Find technology"]'); await page.keyboard.press('Enter');
  await fill(page, '[aria-label="Find technology"]', 'Private SDK'); await page.keyboard.press('Enter');
  const withTechnologies = (await saved(page)).implementation;
  assert.equal(withTechnologies.assignments.length, 2);
  await page.click('[aria-label="Unassign TypeScript"]');
  assert.equal((await saved(page)).implementation.assignments.length, 1);
  await click(page, 'Undo'); assert.deepEqual((await saved(page)).implementation, withTechnologies);
  await click(page, 'Redo'); assert.equal((await saved(page)).implementation.assignments.length, 1);
  await click(page, 'Undo');
  await click(page, 'Implementation', '.designer-lens');
  assert.equal(await page.$$eval('.designer-node-implementation', (nodes) => nodes.length), 6);
  await click(page, 'Structure', '.designer-lens');
  assert.equal(await page.$('.designer-node-implementation'), null);
  assert.equal(await page.$$eval('button', (buttons) => buttons.find((button) => button.textContent === 'Redo')?.disabled), false, 'Lens changes preserve redo');
  await page.type('[aria-label="Find Part"]', 'Private SDK'); await click(page, 'Users', '.designer-search-results');
  await click(page, 'Implementation', '.designer-lens');
  await click(page, 'System context'); await click(page, 'Implementation', '.designer-inspector');
  assert.match(await page.$eval('[aria-label="System technology stack"]', (element) => element.textContent ?? ''), /Private SDK/);
  await page.screenshot({ path: '/tmp/bunkercode-implementation-browser-stack.png', fullPage: true });
  ...
  assert.deepEqual(reopened.implementation, beforeReload.implementation);
  ...
  await page.click('.designer-export summary'); await click(page, 'Export Model JSON'); await page.waitForFunction(() => !!document.documentElement.dataset.exportedModel);
  ...
  await click(page, 'Export Design Document');
  await page.waitForFunction(() => document.documentElement.dataset.exportedModel?.includes('documentVersion'));
  const exportedDocument = await page.$eval('html', (element) => (element as HTMLElement).dataset.exportedModel ?? '');
  assert.deepEqual(JSON.parse(exportedDocument).implementation, (await saved(page)).implementation);
  await click(page, 'Export SVG');
  await page.waitForFunction(() => document.documentElement.dataset.exportedModel?.startsWith('<svg'));
  assert.match(await page.$eval('html', (element) => (element as HTMLElement).dataset.exportedModel ?? ''), /Private SDK/);
  ...
  await fill(page, '[aria-label="Planned System JSON"]', exportedDocument); await click(page, 'Import system');
  ...
  const copyImplementation = await page.evaluate((id) => JSON.parse(localStorage.getItem('bunkercode.design.v1:' + id) ?? '{}').implementation, copy?.id);
  assert.equal(copyImplementation.assignments.length, 0, 'Part deletion cleans technology references');
  ...
  assert.deepEqual(restored.implementation, beforeCascade.implementation);
