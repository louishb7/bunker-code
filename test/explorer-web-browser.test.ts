...
test('Explorer navigates factual territories and focused file relationships in a real browser', { timeout: 90000 }, async (t) => {
  ...
  try {
    ...
    assert.equal(await page.$('[aria-label="System Map inspector"]'), null);
    assert.equal(await page.$$eval('.react-flow__edge', (edges) => edges.length), 0);
    await page.click('.system-map-field-key input[type="checkbox"]');
    assert.equal(await page.$$eval('.react-flow__edge', (edges) => edges.length), 2);
    await page.click('.system-map-field-key input[type="checkbox"]');
    await page.click('[data-system-map-refine="directory:apps"]');
    ...
    assert.equal(await page.$eval('[data-system-map]', (element) => element.getAttribute('data-system-map-item-count')), '3');
    await page.click('[data-system-map-refine="directory:packages"]');
    ...
    const packagesBox = packagesPositions.find((box) => box.id === 'context:directory:packages');
    assert.ok(currentApps && currentTest && packagesBox);
    assert.ok(currentApps.x < packagesBox.x && packagesBox.x < currentTest.x);
    ...
    assert.ok(await page.$('.react-flow__edge'));
    ...
    await page.focus('[data-system-map-refine="directory:packages/graph-engine"]');
    await page.keyboard.press('Enter');
    await page.waitForSelector('[data-system-map-item-id="packages/graph-engine/src/project-graph.ts"]', { timeout: 5000 });
    await page.waitForFunction(() => document.activeElement?.getAttribute('data-system-map-collapse') === 'directory:packages/graph-engine');
    ...
    assert.equal(await page.$eval('[data-system-map-item-id="packages/graph-engine/src/project-graph.ts"]', (element) => element.getAttribute('data-system-map-parent-frame')), 'context:directory:packages/graph-engine/src');
    assert.equal(await page.$eval('[data-system-map-context-frame="directory:packages/graph-engine/src"]', (element) => element.getAttribute('data-system-map-frame-kind')), 'wrapper');
    await captureSystemMap(page, 'graph-engine');
    await page.click('[aria-label="Focus packages/graph-engine"]');
    await page.waitForSelector('[data-map-camera-ready="true"]');
    assert.ok(await page.$('[data-system-map-item-id="directory:apps"]'));
    assert.ok(await page.$('[data-system-map-item-id="directory:test"]'));
    await page.click('[aria-label="Map scale"] > button');
    await page.waitForSelector('[data-map-camera-ready="true"]');
    assert.ok(await page.$('[data-system-map-context-frame="directory:packages/graph-engine"]'));
    assert.equal(await page.$eval('[data-system-map]', (element) => element.getAttribute('data-selected-system-map-item')), '');
    await page.click('[data-system-map-item-id="packages/graph-engine/src/index.ts"] button');
    ...
    await page.waitForFunction(() => document.activeElement?.getAttribute('data-system-map-refine') === 'directory:packages');
    assert.ok(await page.$('[data-system-map-item-id="directory:apps"]'));
    assert.ok(await page.$('[data-system-map-item-id="directory:test"]'));
    ...
    await page.focus('[data-system-map-refine="directory:test"]');
    await page.keyboard.press('Enter');
