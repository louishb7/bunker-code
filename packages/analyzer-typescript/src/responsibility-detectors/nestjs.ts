...
import { decoratorFor, evidenceFor, findingFor, nestAliases, nestApplicable, subjectFor, unsupportedAccessLimitations } from './nestjs-common.js';
...
export const nestjsHttpDetector: ResponsibilityDetector = { detector: { id: 'nestjs.http', version: '1' }, capability: 'http-entry-point', analyze(session) {
  ...
  const findings = [];
  for (const sourceFile of session.sourceFiles.values()) { const aliases = nestAliases(sourceFile); for (const klass of sourceFile.getClasses()) { const controller = decoratorFor(klass, aliases, 'Controller'); if (!controller) continue; for (const method of klass.getMethods()) { const route = method.getDecorators().find((item) => routeDecorators.has(aliases.get(item.getName()) ?? '')); if (route) findings.push(findingFor(subjectFor(session, method), 'http-entry-point', this.detector, 'nestjs-controller-route', [evidenceFor(session, controller), evidenceFor(session, route)])); } } }
...
export const nestjsAccessDetector: ResponsibilityDetector = { detector: { id: 'nestjs.access-control', version: '1' }, capability: 'access-control', analyze(session) {
  ...
  const limitations = unsupportedAccessLimitations(session, this.detector);
  return limitations.length > 0 ? { status: 'partially-evaluated', findings, limitations: limitations as [typeof limitations[number], ...typeof limitations] } : { status: 'evaluated', findings, limitations: [] };
} };
